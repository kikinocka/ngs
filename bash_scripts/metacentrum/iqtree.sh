...
#compute on scratch
cd $SCRATCHDIR
aln='aox.CD.trimal_gt-0.8.aln'
guide='guide_aox'
guide_tree=$guide'.treefile'
bb=1000
nm=5000

iqtree2-mpi -m LG+G4 -T AUTO --threads-max $PBS_NUM_PPN --quiet --safe -s $aln --prefix $guide
iqtree2-mpi -m TEST -madd C10,C20,C30,C40,C50,C60,LG4M,LG4X,LG+F+G,LG+C10+G,LG+C20+G,LG+C30+G,LG+C40+G,LG+C60+G \
	-T AUTO --threads-max $PBS_NUM_PPN -B $bb --nmax $nm --quiet --safe -s $aln --tree-freq $guide_tree --boot-trees
