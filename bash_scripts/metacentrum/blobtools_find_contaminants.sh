...
grep -v "#" $report_dir$base'_contaminants/'$base'_contaminants.tsv' | cut -f1 | sed -e 's/^/grep "/g' -i -e 's/$/" ${blast}/g' > $report_dir$base'_contaminants/'$base'_get_cont_from_blast.sh'
