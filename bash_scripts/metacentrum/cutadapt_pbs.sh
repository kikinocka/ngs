...
cp $data'1_CACTGT_merged.fq.gz' $SCRATCHDIR
...
cutadapt --discard-untrimmed -g ${PRIMER_F} -j $PBS_NUM_PPN -o ${OUT_FW} ${INPUT} > ${LOG}

# Get reads containing reverse primer
cutadapt --discard-untrimmed -a ${PRIMER_R} -j $PBS_NUM_PPN -o ${OUT_RV} ${OUT_FW} >> ${LOG}

# Get reads containing anti-reverse primer (in 5' position)
cutadapt --discard-untrimmed -g ${ANTI_PRIMER_R} -j $PBS_NUM_PPN -o ${OUT_ANTI_FW} ${INPUT} >> ${LOG}

# Get reads containing anti-forward primer (in 3' position)
cutadapt --discard-untrimmed -a ${ANTI_PRIMER_F} -j $PBS_NUM_PPN -o ${OUT_ANTI_RV} ${OUT_ANTI_FW} >> ${LOG}
