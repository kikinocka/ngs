...
cutadapt --discard-untrimmed --format=fastq -g ${PRIMER_F} -o ${OUT_FW} ${INPUT} > ${LOG}

# Get reads containing reverse primer
cutadapt --discard-untrimmed --format=fastq -a ${PRIMER_R} -o ${OUT_RV} ${OUT_FW} >> ${LOG}

# Get reads containing anti-reverse primer (in 5' position)
cutadapt --discard-untrimmed --format=fastq -g ${ANTI_PRIMER_R} -o ${OUT_ANTI_FW} ${INPUT} >> ${LOG}

# Get reads containing anti-forward primer (in 3' position)
cutadapt --discard-untrimmed --format=fastq -a ${ANTI_PRIMER_F} -o ${OUT_ANTI_RV} ${OUT_ANTI_FW} >> ${LOG}
