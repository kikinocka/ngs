...
os.chdir('/home/kika/MEGAsync/diplonema_mt/1601/transcripts/y3/')
xml = open('y3_blast.xml')
...
c = 0
with open('y3_modules.txt', 'w') as modules:
	for item in sorted(coordinates.items()):
		c += 1
		for seq in SeqIO.parse('y3_contigs.txt', 'fasta'):
