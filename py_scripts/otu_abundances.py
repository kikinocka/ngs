...
df = pd.read_csv('otu_table.updated.tsv', sep='\t')


df[['rank1', 'rank2', 'rank3']] = df.lineage.str.split('|', 2, expand=True)

# #supergroups
# total = df.groupby(['rank2']).sum().filter(regex='V9.*', axis=1)
# total = total.T
# print(total)
# print(total.columns)
# print(total.size)
# total = total.drop('V9-Oct10-2018-P3S_S72', axis=0)

# total = total[['No_hit', 'Eukaryota_X', 'Cryptista', 'Haptista', 'Stramenopiles', 'Alveolata', 'Rhizaria',
...
meta['species'] = meta.lineage.apply(lambda x: x.split("|")[-1])
meta = meta.groupby(['species']).sum().filter(regex='V9.*', axis=1)
meta = meta.T
# print(meta)
...
ax = meta.plot(kind='barh', stacked=True, color=colors, width=0.5, align='center')
...
plt.tight_layout()
plt.show()
# plt.savefig('metamonads.pdf', dpi=300)
