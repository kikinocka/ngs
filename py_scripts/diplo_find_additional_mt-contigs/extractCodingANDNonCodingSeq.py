...
def main() :
    ...
    if ARGS.out :
        ...
        out      = os.path.basename(ARGS.fasta).split(".")[0] + "_Cod+NonCod.fasta"
    #print "=>Output extracted sequence file of features: " + out

    lgff   = readGFF( ARGS.gff ) # List of genome gff lines
    dgfffeat    = filterGFF( lgff, ["mRNA","rRNA"] )   # sublist of selected features from genome gff lines
    lseq        = readFasta( ARGS.fasta )       # dico of sequence from genome 

    foutfasta   = open(out, 'w')
    ...
    for seq in lseq :
        ...
        #print seq[ID]
        #print seqATGC
        if not seq[ID] in dgfffeat :
            continue

        lgfffeat    = sorted( dgfffeat[ seq[ID] ], key=lambda k: k[START] )
        start       = 1             # previous element start
        end         = len(seqATGC)
        i           = 0
        for feat in lgfffeat :
            i += 1
            # Non coding
            if feat[START] - start >= 50 :
                foutfasta.write(">nc%s_%i\tNon coding\t%s[%i-%i]\n"%(seq[ID], i, seq[ID], start, feat[START]) ) 
                foutfasta.write(seqATGC[start-1:feat[START]] + "\n" )

                #print  "%s\tNon coding\t%i-%i"%(seq[ID], start, feat[START])    
                #print  "Seq: " + seqATGC[start-1:feat[START]]
                #print  "%s\t%s\t%i-%i"%(seq[ID], feat[FEATURE],feat[START], feat[END])   
                nfeat += 1

            # Coding
            i += 1
            foutfasta.write(">%s\tCoding\t%s[%i-%i]\n"%(feat[ID], seq[ID], feat[START], feat[END]) )
            foutfasta.write(seqATGC[feat[START]-1:feat[END]+1] + "\n" )
            nfeat += 1
        ...
        if end - start >= 50 :
            i += 1
            foutfasta.write(">nc%s_%i\tNon coding\t%s[%i-%i]\n"%(seq[ID], i, seq[ID], start, end) )
            foutfasta.write(seqATGC[start-1:end+1] + "\n" )
            #print  "%s\tNon coding\t%i-%i"%(seq[ID], start, end)
            #print  "Seq: " + seqATGC[start-1:end]
...
def readGFF( fgff ) :
    ...
    for line in f :
        ...
        for a in latt :
            #ID
            match     = patid.search(a)
            if match :
                gff[ID] = match.group(1)
            #PARENT
            match     = patparent.search(a)
            if match :
                gff[PARENT] = match.group(1)
            #NOTE
            match     = patnote.search(a)
            if match :
                gff[NOTE] = match.group(1)
        lgff.append(gff)
    return lgff
...
def filterGFF( lgff, lfeatname ) :
    ...
    for feat in lfeatname :
        lfeatlower.append( feat.lower() )

    n               = 0
    dfeatgff        = {}
    for g in lgff :
        if g[FEATURE].lower() in lfeatlower :
            if not g[SEQ] in dfeatgff :
                dfeatgff[ g[SEQ] ] = []
            dfeatgff[ g[SEQ] ].append(g)
            n += 1

    print "[IN] %i features found in GFF file" %(n,)
    return dfeatgff
